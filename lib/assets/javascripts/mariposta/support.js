...
Mariposta.Support = {
  ...
  setupPageEditor: function(options) {
    var _self = this;
    ...
    var defaultOptions = {
      ...
      toolbar: ["bold", "italic", "code", "heading-2", "heading-3", "|", "quote", "unordered-list", "ordered-list", "|", "link", "horizontal-rule", "|", "fullscreen", {
          ...
          action: function customFunction(editor){
            _self.destroyPageEditor()
          },
          className: "fas fa-code",
          title: "Switch to Raw Source",
        }
      ]
    }
    ...
    if ($('div.CodeMirror').length == 0) {
      $('#mariposta-switch-to-rich-editor').remove()
      this.simplemde = new SimpleMDE(options)
    }
  },
  destroyPageEditor: function() {
    var _self = this;

    this.simplemde.toTextArea()
    this.simplemde = null
    $("#page-editor-box").before("<div id=\"mariposta-switch-to-rich-editor\"><button class=\"button small\">Switch to Rich Editor</button></div>")
    $('#mariposta-switch-to-rich-editor > button').click(function(e) {
      e.preventDefault()
      _self.setupPageEditor()
    })
  },
  ...
  cloudinaryStyles: function() {
    var primaryColor = $('button.cloudinary-selector').css('background-color')
